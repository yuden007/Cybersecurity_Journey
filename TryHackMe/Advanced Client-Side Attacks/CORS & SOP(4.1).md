...
- **Simple Requests**: Use GET, HEAD, or POST with standard Content-Types and no custom headers. Sent directly with the Origin header; browser enforces CORS based on the server's Access-Control-Allow-Origin response.
- **Preflight Requests**: For other methods, custom headers, or non-standard Content-Types, the browser first sends an OPTIONS request (the "preflight") to check if the server allows the actual request. If allowed, the real request follows.
...
The Access-Control-Allow-Origin (ACAO) header tells browsers which origins can access a resource. When a cross-origin request is made, the server checks the Origin and, if allowed, responds with the ACAO header specifying the permitted origin or * for any origin.
...
- **Multiple Origins:**
  Dynamically set ACAO for allowed origins. Allows a specific list of trusted origins.
...
- **With Credentials:**
  ACAO must be a specific origin + Access-Control-Allow-Credentials: true. Allows cookies/auth data in cross-origin requests.
...
- **Accepting 'null' origins:** This allows requests from local files or data URLs, which can be abused by attackers. Always reject 'null' unless explicitly needed.
- **Weak origin validation (bad regex):** Poorly written regex can match unintended domains (e.g., /example.com$/ matches badexample.com). Use strict, tested patterns.
- **Reflecting arbitrary Origin headers:** Echoing back any Origin in Access-Control-Allow-Origin lets any site access protected resources. Only allow trusted origins from a defined allowlist.
...
- Otherwise, it rejects the request, ensuring only allowlisted origins are allowed. This method minimizes the risk of CORS-related vulnerabilities.
...
1. Open http://corssop.thm/exploits/data_exfil.html on the exploit hosting server.
2. Change the target URL to: http://corssop.thm/arbitrary.php
3. Change the exfiltration receiver URL to your attacker server (e.g., ATTACKER_IP:81/receiver.php for AttackBox Apache).
4. Save the page on the exploit server.

## Verify the hosted exploit
...
- If the target response contains sensitive data (cookies, tokens, user data), the exploit can capture and send it to the attacker server. Proper origin validation or an allowlist prevents this.
...
1. Open http://corssop.thm/exploits/data_exfil.html on the exploit hosting server.
2. Set the target URL to: http://corssop.thm/badregex.php
3. Set the exfiltration receiver URL to your attacker server (e.g., ATTACKER_IP:81/receiver.php).
4. Save the page on the exploit server.
...
1. Host an exploit page (data URL or local file) that loads a hidden iframe which runs JavaScript to request the vulnerable endpoint (http://corssop.thm/null.php) with credentials.
2. Set the exfiltration receiver to your attacker server (e.g., http://EXFILTRATOR_IP/receiver.php).
