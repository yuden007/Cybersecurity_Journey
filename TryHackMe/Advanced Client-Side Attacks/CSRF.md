...
SameSite cookies are a key defense against CSRF. The SameSite attribute controls when cookies are sent with cross-site requests for preventing CSRF; Lax allows some cross-site GETs; None is for cross-origin use but must be Secure.
...
Attacker's goal: log Josh out by exploiting the Lax SameSite cookie.

The logout cookie is set as Lax, so it is sent with top-level navigation GET requests. The attacker sends Josh a survey link:
