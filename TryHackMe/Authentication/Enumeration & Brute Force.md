...
Using grep on normal output lacks the host's IP, e.g., `80/tcp open http nginx 1.6.2`, making it less useful for multiple scans. Grepable output includes the IP, providing complete information per line.
...
The third format is XML, saved with `-oX FILENAME`. Use `-oA FILENAME` to save in all three formats: normal, grepable, and XML.
