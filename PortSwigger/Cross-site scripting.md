...
### WEAKNESS:
Unsanitized search input is executed as JavaScript.

### SOLUTION:
Enter <script>alert(1)</script> in the search box.
...
### WEAKNESS:
Unsanitized comment input is executed as JavaScript

### SOLUTION:
Enter <script>alert(1)</script> as a comment.
...
### WEAKNESS:
Unsanitized data from location.search is written into the DOM via document.write

### SOLUTION:
Enter a random alphanumeric string into the search box.
Inspect the element, and observe that your random string has been placed inside an img src attribute. 
...
### WEAKNESS:
Unsanitized input from location.search is written into the DOM via innerHTML

### Solution:
Enter the following into the into the search box, <img src=1 onerror=alert(1)>
The invalid src triggers the onerror event handler, which then calls the alert() function. 
...
### WEAKNESS:
Unsanitized input from location.search is written into the DOM via innerHTML.
href is able to execute javascript.
...
### WEAKNESS:
Vulnerable jQuery version with $() used with unsanitized location.hash input.
$(location.hash) is able to create html element.

### SOLUTION:
In the exploit server's body section, add the following malicious iframe:
<iframe src="https://YOUR-LAB-ID.web-security-academy.net/#" onload="this.src+='<img src=x onerror=print()>'"></iframe>
Store the exploit, then click View exploit to confirm that the print() function is called. Submit.
...
### WEAKNESS:
Quoted attribute is breakable.

### SOLUTION:
Submit a random alphanumeric string in the search box, then use Burp Suite to intercept the search request.
Inspect the element, and observe that your random string has been placed inside an img src attribute. 
Replace your input with "onmouseover="alert(1)
...
Inspect the element, and observe that the random string has been reflected inside an anchor href attribute.
Replace your input with the following payload: javascript:alert(1)
...
### WEAKNESS:
Payload breaks out of javascript string and execute javascript code.

### SOLUTION:
Post a comment with a random alphanumeric string in the "Website" input.
Inspect the element, and observe that the random string has been reflected inside a JavaScript string. 
Replace your input with the following payload: '-alert(1)-'
Reload the page should trigger an alert.
...
### WEAKNESS:
Unsanitized storeId parameter injected into DOM via document.write()

### OBSERVATION:
On the product pages, notice that the dangerous JavaScript extracts a storeId parameter with location.search(). 
